...
import PurchasePackage from '../components/registration/PurchasePackage';
...
const Register: React.FC = () => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch(`${apiUrl}/upload/public`, { method: 'POST', body: formData });
      const data = await response.json();
      if (!response.ok || !data.success) throw new Error(data.message || 'File upload failed');
  ...
  // Step 2 (sitter listing) → Step 3
  const handleContinueFromListing = (payload: any) => {
    setListingPayload(payload);
    setRegistrationStep(3);
  };

  // Step 2 (owner package) → Step 3
  const handlePackageSelected = (packageType: 'monthly' | 'annual') => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/auth/register`, {
        ...
        body: JSON.stringify({
          ...
          verificationReport,
          policeVerification,
          // Include package info for owners
          packagePurchased: role === 'owner' && selectedPackage !== null,
          packageType: role === 'owner' ? (selectedPackage || '') : '',
        }),
      });
      ...
      if (data.user.role === 'sitter') {
        ...
        // Create listing if payload exists
        if (listingPayload) {
        ...
        setSuccess('Registration complete! Redirecting to your dashboard...');
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {registrationStep === 1 && (
          <div className="max-w-[540px] mx-auto">
            ...
            {error && <div className="mb-6 p-4 bg-red-50 border border-red-100 text-red-600 text-xs rounded-md">{error}</div>}
            {success && <div className="mb-6 p-4 bg-green-50 border border-green-100 text-green-600 text-xs rounded-md">{success}</div>}
            ...
            <form className="space-y-5" onSubmit={handleNextStep1}>
              ...
              <p className="text-[10px] text-slate-400 uppercase tracking-widest pt-2">
                Step 1 of 3 — {role === 'owner' ? 'Basic Info → Package → Verify' : 'Basic Info → Listing → Verify'}
              </p>
        ...
        {/* ── STEP 2 for SITTER: Add Listing ── */}
        {registrationStep === 2 && role === 'sitter' && (
          <div className="w-full animate-in fade-in zoom-in duration-500">
            <SellerAddListing isRegistrationFlow={true} onContinueRegistration={handleContinueFromListing} />
          </div>
        )}

        {/* ── STEP 2 for OWNER: Purchase Package ── */}
        {registrationStep === 2 && role === 'owner' && (
          <PurchasePackage
        ...
        {registrationStep === 3 && (
          <div className="max-w-[540px] mx-auto animate-in slide-in-from-right duration-500">
            ...
            <div className="flex gap-10 border-b border-slate-100 mb-10">
              ...
              {role === 'owner' && selectedPackage && (
                <span className="pb-4 text-[11px] text-[#788564] font-medium self-end ml-auto">
                  ✓ {selectedPackage === 'monthly' ? 'Monthly R190/mo' : 'Annual R1900/yr'} selected
                </span>
              )}
            </div>
              ...
              <div className="p-5 bg-slate-50 border border-slate-200 rounded-xl space-y-4 mt-6">
                ...
                {isVerifiedClicked && (
                  <div className="space-y-1.5 pt-3 border-t border-slate-200/60">
                    ...
                    {verificationReport && (
                      <p className="text-[10px] text-[#5A7E49] font-semibold flex items-center gap-1">✓ Report uploaded successfully</p>
                    )}
                  </div>
                )}
              </div>

              {/* Police Verification — sitters only */}
              {role === 'sitter' && (
                <div className="p-5 bg-slate-50 border border-slate-200 rounded-xl space-y-4 mt-6">
                  <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                    <div>
                  ...
                  <div className="space-y-1.5 pt-3 border-t border-slate-200/60">
                    ...
                    {policeVerification && (
                      <p className="text-[10px] text-slate-600 font-semibold flex items-center gap-1">✓ Police verification uploaded successfully</p>
                    )}
                  </div>
                </div>
              )}
