...
import {
  Search, Send, Paperclip, CheckCheck, Circle, Info
} from 'lucide-react';
...
const SellerMessagesSection = () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const threadsData = await Promise.all(bookingsList.map(async (b: any) => {
          ...
          return {
            id: b._id,
            userName: b.client ? `${b.client.firstName || ''} ${b.client.lastName || ''}`.trim() || b.client.username : 'Unknown Client',
            userAvatar: getImageUrl(b.client?.avatar),
            userStatus: 'Online' as const,
            petInfo: `${b.petCount} Pet(s) - ${b.listing?.title || 'Pet Care'}`,
  ...
  // Fetch once when switching thread
  useEffect(() => {
    if (activeThreadId) fetchActiveMessages();
  }, [activeThreadId]);

  // ── Socket.io real-time for active thread ──────────────
  useEffect(() => {
    if (!activeThreadId) return;
    const socket = getSocket();
    socket.emit('join_booking', activeThreadId);
    ...
    return () => {
      socket.emit('leave_booking', activeThreadId);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await fetch(`${apiUrl}/messages`, {
        ...
        body: JSON.stringify({ bookingId: activeThreadId, content: text }),
      });
      // Socket.io 'new_message' event will handle adding message to state
    } catch (e) {
  ...
  const filteredThreads = threads.filter(t =>
    ...
    t.petInfo.toLowerCase().includes(searchQuery.toLowerCase())
  );

  return (
    <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden flex h-[620px]">

      {/* 1. Inbox Left Sidebar: Threads list */}
      <div className="w-[340px] border-r border-slate-100 flex flex-col h-full bg-[#fcfcfc]/30">
        ...
        <div className="p-4 border-b border-slate-50 space-y-3">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search chat or pet info..."
              value={searchQuery}
                ...
                >
                  ...
                  <div className="relative flex-shrink-0">
                    ...
                    {thread.userStatus === 'Online' && (
                      <span className="absolute bottom-0 right-0 w-3 h-3 bg-[#5cb85c] rounded-full border-2 border-white shadow-sm" />
                    )}
                  </div>
      ...
      <div className="flex-1 flex flex-col h-full bg-white relative">
        {activeThread ? (
          <>
            {/* Chat header */}
            <div className="p-4 border-b border-slate-50 flex items-center justify-between flex-shrink-0 shadow-sm/5 z-10">
            ...
            <div className="bg-slate-50/50 border-b border-slate-50 px-6 py-2.5 flex items-center gap-2 text-[11px] text-slate-500 flex-shrink-0">
              ...
              <span>Discussing Booking: <strong className="text-slate-700 font-bold">{activeThread.petInfo}</strong></span>
            </div>
...
export default SellerMessagesSection;
